...
// Función auxiliar para seleccionar carta aleatoria
function getRandomCard(cardPool) {
    if (!cardPool || cardPool.length === 0) return null;
    return cardPool[Math.floor(Math.random() * cardPool.length)];
}

// Función auxiliar para obtener rara o mítica (13.51% mítica, 86.49% rara)
...
// Simular apertura de sobre siguiendo estructura oficial de Magic
export async function openBooster(setCode, setInfo = {}) {
    try {
        ...
        // ESPACIOS 10-11: Wildcards (2 cartas de rareza aleatoria)
        for (let i = 0; i < 2; i++) {
        ...
        // ESPACIOS 12-14: 3 Raras o Míticas (13.51% mítica cada una)
        for (let i = 0; i < 3; i++) {
